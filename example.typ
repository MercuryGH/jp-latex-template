...
To use ruby-typ, you should import the `get-ruby` from `ruby.typ` at first.

```typ
#import "ruby.typ": get-ruby 
```

`get-ruby` is a higher-order function which be used to configure ruby-typ. and it will return a function for ruby. and its function signature is
```typc
get-ruby(
...
```typ
#let ruby = get-ruby(alignment: "start")
#ruby("しずく", "雫")
```
